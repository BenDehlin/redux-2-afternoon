...
const REMOVE_PURCHASE = 'REMOVE_DATA'
...
export function addPurchase(price, description, category){
  const data = axios.post('/api/budget-data/purchase', {price, description, category})
...
export function removePurchase(id){
  const data = axios.delete(`/api/budget-data/purchase/${id}`).then(results => results.data).catch(err => console.log(err))
  return {
